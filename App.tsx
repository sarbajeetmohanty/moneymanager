...
const AppContext = createContext<{
  ...
  refreshKey: number;
  transactionMode: 'History' | 'Add';
  setTransactionMode: (mode: 'History' | 'Add') => void;
}>({ refreshAll: () => {}, showToast: () => {}, refreshKey: 0, transactionMode: 'History', setTransactionMode: () => {} });
...
export const App: React.FC = () => {
  ...
  const [transactionMode, setTransactionMode] = useState<'History' | 'Add'>('History');
  ...
  return (
    <AppContext.Provider value={{ refreshAll, showToast, refreshKey, transactionMode, setTransactionMode }}>
      <div className={`flex flex-col h-screen overflow-hidden transition-all duration-700 ${presetClass}`}>
        ...
        {toast && (
          <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[300] animate-in slide-in-from-top-12 fade-in duration-500">
          ...
          <div className="fixed inset-0 z-[400] flex items-center justify-center bg-slate-950/80 backdrop-blur-xl p-6">
            <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[3.5rem] p-12 shadow-2xl animate-in zoom-in-95 duration-500 text-center border border-white/5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <button onClick={() => setShowLogoutModal(false)} className="py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest active:scale-95 transition-all">Stay In</button>
                <button onClick={confirmLogout} className="bg-red-600 text-white py-5 rounded-3xl shadow-xl text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all">Logout</button>
              </div>
            </div>
          </div>
        )}
