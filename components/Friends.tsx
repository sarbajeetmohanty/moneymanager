...
export const Friends: React.FC<FriendsProps> = ({ user }) => {
  ...
  const removeFriend = async (fId: string, balance: number) => {
    ...
    if (!window.confirm("This will remove the friend from your list. Past transactions will remain visible. Continue?")) return;
        ...
        <div className="space-y-6 flex-grow overflow-y-auto no-scrollbar pb-32 px-2">
          ...
          <div className="space-y-4">
            {filteredFriends.length === 0 ? (
               <div className="card-ui bg-white dark:bg-slate-900 p-20 text-center border-2 border-dashed border-slate-100 dark:border-slate-800 opacity-50">
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">No matching connections</p>
               </div>
      ...
      {showConnect && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/90 backdrop-blur-xl p-8">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[3.5rem] p-12 shadow-2xl animate-in zoom-in-95">
            <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-8">New Connection</h2>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 ml-4">Enter Username or Email</p>
            <input 
              type="text" 
              placeholder="Username / Email" 
              className="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-3xl p-6 mb-10 font-black text-sm dark:text-white focus:ring-2 focus:ring-indigo-500"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <button onClick={handleConnect} className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-5 rounded-[24px] text-[10px] font-black shadow-lg uppercase tracking-widest active:scale-95 transition-all">Request</button>
            </div>
          </div>
        </div>
      )}
...
const FriendRow = ({ friend, onClick }: any) => (
  <div onClick={onClick} className="card-ui bg-white dark:bg-slate-900 p-6 shadow-sm flex items-center justify-between active:scale-[0.98] transition-all cursor-pointer group">
    <div className="flex items-center gap-5">
      <div className="w-16 h-16 rounded-[24px] bg-slate-50 dark:bg-slate-800 flex items-center justify-center font-black text-slate-300 dark:text-slate-600 text-2xl group-hover:scale-110 transition-transform shadow-inner">
        {friend.name[0]}
      </div>
      <div>
...
const FriendDetail = ({ friend, onBack, userId, user, onRemove }: any) => {
  const { showToast } = useApp();
  const [history, setHistory] = useState<any[]>([]);
  const [subTab, setSubTab] = useState<'Activity' | 'Unsettled'>('Activity');

  const fetchHistory = async () => {
    const res = await api.fetchTransactionHistory(userId);
    if(res.success) {
      setHistory(res.history.filter((t: any) => t.friendId === friend.id || t.creatorId === friend.id || t.participants?.some((p:any) => p.userId === friend.id)));
    }
  };
  ...
  const sendReminder = async (tId: string) => {
    ...
    if (res.success) {
      showToast("Reminder sent to " + friend.name, "success");
    }
  };
  ...
  return (
    <div className="h-full flex flex-col animate-in slide-in-from-right duration-500 no-scrollbar pb-32 overflow-y-auto px-2">
      <div className="flex items-center justify-between mb-8 sticky top-0 z-10 py-4 bg-slate-50/80 dark:bg-slate-950/80 backdrop-blur-md">
        ...
        <h3 className="font-black text-slate-900 dark:text-white text-[10px] uppercase tracking-[0.3em]">{friend.name} Detail</h3>
      ...
      <div className="card-ui bg-white dark:bg-slate-900 p-12 text-center shadow-sm mb-10">
         <div className="w-28 h-28 rounded-[36px] bg-slate-50 dark:bg-slate-800 mx-auto mb-8 flex items-center justify-center text-slate-300 dark:text-slate-600 font-black text-4xl uppercase shadow-inner">{friend.name[0]}</div>
         ...
         <p className={`text-[11px] font-black uppercase tracking-widest ${friend.balance === 0 ? 'text-slate-400' : friend.balance > 0 ? 'text-green-600' : 'text-red-500'}`}>
            {friend.balance === 0 ? 'No Dues' : friend.balance > 0 ? `Will pay you: ₹${friend.balance.toLocaleString()}` : `You need to pay: ₹${Math.abs(friend.balance).toLocaleString()}`}
         </p>
      </div>

      <div className="flex bg-white dark:bg-slate-900 p-2 rounded-full mb-10 shadow-sm">
        {['Activity', 'Unsettled'].map(tab => (
          <button 
            key={tab}
            onClick={() => setSubTab(tab as any)}
            className={`flex-1 py-4 text-[9px] font-black uppercase tracking-widest rounded-full transition-all ${subTab === tab ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-xl' : 'text-slate-400'}`}
          >
            {tab}
          </button>
        ))}
      ...
      <div className="space-y-4">
        {filteredHistory.length === 0 ? (
          <div className="text-center py-20 opacity-30"><p className="text-[10px] font-black uppercase tracking-widest">No history with this friend</p></div>
        ) : filteredHistory.map(t => (
          <div key={t.id} className="card-ui bg-white dark:bg-slate-900 p-6 flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-5">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm ${t.type.includes('Given') ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                <i className={`fa-solid ${t.type.includes('Given') ? 'fa-arrow-up' : 'fa-arrow-down'}`}></i>
              </div>
              <div>
                <p className="text-[11px] font-black text-slate-800 dark:text-white uppercase tracking-tight">{t.category}</p>
            ...
            <div className="text-right flex flex-col items-end gap-2">
              <p className={`text-base font-black ${t.type.includes('Given') ? 'text-green-600' : 'text-red-500'}`}>₹{t.amount.toLocaleString()}</p>
              {(t.status === 'Pending' || t.status === 'Paid') && (
                <button onClick={() => sendReminder(t.id)} className="bg-indigo-600 text-white px-4 py-2 rounded-full text-[8px] font-black uppercase tracking-widest active:scale-90 transition-all">Remind</button>
              )}
            </div>
          </div>
        ))}
