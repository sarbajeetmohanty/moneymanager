...
export const Friends: React.FC<FriendsProps> = ({ user }) => {
  ...
  const handleConnect = async () => {
    ...
    if (res.success) {
      showToast("Link Request Dispatched", "success");
  ...
  const removeFriend = async (fId: string, balance: number) => {
    // Aggregated balance check as requested
    if (Math.abs(balance) > 0.01) {
      alert(`STRICT BLOCK: You cannot remove this friend until all dues are settled. Current balance: ₹${Math.abs(balance).toLocaleString()}. Please settle first!`);
      return;
    }
    const confirmed = window.confirm("Final check: Remove this connection? History will remain archived.");
    if (!confirmed) return;
    const res = await api.removeFriend(user.id, fId);
    if (res.success) {
      showToast("Connection Terminated", "success");
      fetchFriends();
      ...
      {selectedFriend ? (
        ...
        <div className="space-y-6 flex-grow overflow-y-auto no-scrollbar pb-32 px-2">
          <div className="card-ui bg-white dark:bg-slate-900 p-6 shadow-sm flex items-center gap-4">
             ...
             <input type="text" placeholder="Find a friend..." className="flex-1 text-sm font-black bg-transparent border-none focus:ring-0 placeholder:text-slate-300 dark:text-white" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          <div className="flex items-center justify-between px-3">
            <h3 className="font-black text-slate-400 text-[10px] uppercase tracking-[0.3em]">Direct Links</h3>
            <button onClick={() => setShowConnect(true)} className="text-indigo-600 dark:text-indigo-400 font-black text-[10px] uppercase tracking-widest bg-indigo-50 dark:bg-indigo-500/10 px-5 py-2.5 rounded-full active:scale-90 transition-all shadow-sm">+ Add Partner</button>
          </div>
          <div className="space-y-4">
            {filteredFriends.length === 0 ? (
               <div className="card-ui bg-white dark:bg-slate-900 p-20 text-center border-2 border-dashed border-slate-100 dark:border-slate-800 opacity-50"><p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">No active partners</p></div>
      ...
      {showConnect && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/90 backdrop-blur-xl p-8">
          <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[3.5rem] p-12 shadow-2xl animate-in zoom-in-95">
            <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-8">Establish Link</h2>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 ml-4">Email or Handle</p>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <button onClick={handleConnect} className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-5 rounded-[24px] text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all">Link</button>
            </div>
          </div>
        </div>
      )}
    ...
    <div className="flex items-center gap-5">
      ...
      <div>
        ...
        <p className={`text-[10px] font-black uppercase tracking-widest mt-1.5 ${friend.balance > 0 ? 'text-green-600' : friend.balance < 0 ? 'text-red-500' : 'text-slate-300'}`}>
          {friend.balance > 0 ? `Will pay you ₹${friend.balance.toLocaleString()}` : friend.balance < 0 ? `You owe them ₹${Math.abs(friend.balance).toLocaleString()}` : 'Settled'}
        </p>
      </div>
    </div>
...
const FriendDetail = ({ friend, onBack, userId, onRemove }: any) => {
  ...
  const [subTab, setSubTab] = useState<'History' | 'Unpaid'>('History');
  ...
  const filteredHistory = useMemo(() => {
    if (subTab === 'History') return history;
  ...
  return (
    <div className="h-full flex flex-col animate-in slide-in-from-right duration-500 pb-32 overflow-y-auto px-2">
      <div className="flex items-center justify-between mb-8 py-4 bg-slate-50/80 dark:bg-slate-950/80 backdrop-blur-md sticky top-0 z-10">
        ...
        <h3 className="font-black text-slate-900 dark:text-white text-[10px] uppercase tracking-[0.3em]">Node Ledger</h3>
        <button onClick={onRemove} className="w-14 h-14 rounded-2xl bg-red-50 dark:bg-red-500/10 text-red-500 flex items-center justify-center active:scale-90 transition-all"><i className="fa-solid fa-user-xmark"></i></button>
      </div>
      <div className="card-ui bg-white dark:bg-slate-900 p-12 text-center shadow-sm mb-10">
         ...
         <p className={`text-[11px] font-black uppercase tracking-widest ${friend.balance === 0 ? 'text-slate-400' : friend.balance > 0 ? 'text-green-600' : 'text-red-500'}`}>
            {friend.balance === 0 ? 'Fully Settled' : friend.balance > 0 ? `Awaiting: ₹${friend.balance.toLocaleString()}` : `Owing: ₹${Math.abs(friend.balance).toLocaleString()}`}
         </p>
      </div>
      <div className="flex bg-white dark:bg-slate-900 p-2 rounded-full mb-10 shadow-sm">
        {['History', 'Unpaid'].map(tab => (
          ...
          <div key={t.id} className="card-ui bg-white dark:bg-slate-900 p-6 flex items-center justify-between shadow-sm border border-slate-50 dark:border-slate-800">
            <div className="flex items-center gap-5">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm ${t.status === 'Completed' ? 'bg-slate-50 text-slate-400' : 'bg-indigo-50 text-indigo-600'}`}>
                <i className={`fa-solid ${t.status === 'Completed' ? 'fa-check' : 'fa-clock'}`}></i>
              </div>
              <div>
