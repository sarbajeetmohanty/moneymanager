    ...
    <div className="space-y-10 pb-48 no-scrollbar animate-in fade-in duration-1000">
      ...
      <div className="card-ui p-12 bg-slate-900 dark:bg-black text-white shadow-[0_50px_100px_-20px_rgba(0,0,0,0.4)] relative overflow-hidden group">
        <div className="relative z-10">
          <p className="text-white/60 text-[10px] font-black uppercase tracking-[0.5em] mb-4">Total Liquid Balance</p>
          <h2 className="text-7xl font-black tracking-tighter mb-12">₹{totalLiquid.toLocaleString()}</h2>
          
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            <MetricBox label="In Hand (Cash)" value={stats.cash} color="text-emerald-400" />
      ...
      <div className="space-y-6 px-2">
        <div className="flex bg-white/60 dark:bg-slate-900/60 backdrop-blur-3xl p-2 rounded-[3rem] shadow-xl">
          {['Today', 'Yesterday', '7D', '30D', 'Custom'].map(f => (
            <button
              ...
              className={`flex-1 py-4 text-[10px] font-black uppercase tracking-widest rounded-full transition-all duration-300 ${timeFilter === f ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-2xl scale-110' : 'text-slate-400'}`}
            >
              {f}
            </button>
          ))}
        ...
        {timeFilter === 'Custom' && (
          <div className="flex gap-3 animate-in slide-in-from-top-4 duration-500">
            <input 
              type="date" 
              className="flex-1 bg-white dark:bg-slate-900 p-5 rounded-3xl text-[10px] font-black uppercase tracking-widest text-slate-500 shadow-sm"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 px-2">
        ...
        <div className="card-ui bg-white dark:bg-slate-900 p-10 shadow-2xl">
          <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-10 ml-2">Cash Flow Analysis</h3>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={cashFlowData}>
                ...
                <Tooltip />
                <Area type="monotone" dataKey="inflow" stroke="#10b981" fill="#10b98133" strokeWidth={4} />
                <Area type="monotone" dataKey="outflow" stroke="#f43f5e" fill="#f43f5e33" strokeWidth={4} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <AggregateCard label="Incoming" value={stats.incoming} color="text-green-600" />
          <AggregateCard label="Outgoing" value={stats.outgoing} color="text-red-600" />
...
const MetricBox = ({ label, value, color }: any) => (
  <div className="bg-white/5 p-6 rounded-[2.25rem] hover:bg-white/10 transition-all duration-300">
    <p className="text-[9px] text-white/50 font-black uppercase tracking-widest mb-2">{label}</p>
    <p className={`text-lg font-black ${color}`}>₹{(value || 0).toLocaleString()}</p>
  </div>
);

const AggregateCard = ({ label, value, color }: any) => (
  <div className="card-ui bg-white dark:bg-slate-900 p-8 shadow-sm flex flex-col justify-center">
